...
public class ClientsSuppliersData implements CommandLineRunner {
    ...
    for (Long userId : userIds) {
        ...
        if (userOptional.isPresent()) {
            // Create Client
            Client client = new Client(clientNames.get(i),clientLastNames.get(i),clientPhones.get(i),clientAges.get(i),clientGenders.get(i),userOptional.get());
            clientRepository.save(client);
            //Create Address and set Client
            Address address = new Address(addressLine_1.get(i),addressLine_2.get(i),addressCity.get(i),addressState.get(i),addressPostalCode.get(i),client);
            addressRepository.save(address);
            // Create Supplier
            Supplier supplier = new Supplier(businessNames.get(i),supplierPhones.get(i),supplierAddresses.get(i),userOptional.get());
